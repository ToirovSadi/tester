...
// just to avoid compilation error, these three lines are useless #remove
namespace test_generator{void test_generator(ostream& out){}} // #remove
namespace main_program{void main_program(ostream& out, istream& in){}} // #remove
namespace target_program{void target_program(ostream& out, istream& in){}} // #remove
...
void save_test(stringstream& input, stringstream& target_fout, stringstream& fout_main){
    ...
    int ok = 1;
    while(ok){
        string id = get_randstr() + "_" + to_string(saved_test);
